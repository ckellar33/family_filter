...
define_class!(
  ...
  unsafe impl UISceneDelegate for TaoSceneDelegate {
    ...
    // Leaving `Event::Resumed`/`Event::Suspended` unwired here left them dead
    // on arrival for any app that adopts the scene lifecycle (required on
    // iOS 27+, see UIApplicationSceneManifest): once a scene manifest is
    // present, iOS routes background/foreground transitions through these
    // scene-delegate methods instead of the old
    // `application(Will|Did)(ResignActive|EnterBackground|EnterForeground)`
    // AppDelegate callbacks, and `will_resign_active`/`will_enter_foreground`
    // in view.rs -- which emit exactly these events -- simply stop being
    // called. Consumers relying on `Resumed` to redo work sockets/timers
    // don't survive backgrounding (Tauri's own iOS `RunEvent::Resumed` docs
    // suggest exactly this use) silently never see it fire again after the
    // first background/foreground cycle once scenes are on.
    //
    // Mirrors view.rs's non-scene pairing exactly: `Suspended` on
    // will-resign-active (not did-enter-background, which the AppDelegate
    // path leaves a no-op too -- resign-active already fires on every
    // transition out of the foreground, background-bound or not, e.g.
    // Control Center or an incoming call, same as it always has), `Resumed`
