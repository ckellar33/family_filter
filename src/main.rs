...
async fn control_flow() -> Result<(), &'static str> {
    ...
    let keys = match hap_pair::pair_verify(&mut stream, &saved.creds).await {
        Ok(keys) => {
    ...
    loop {
        println!("--- Control ---");
        println!("1) Mute (volume 0)");
        println!("2) Unmute (restore volume)");
        ...
        match input.trim() {
            "1" => match session.mute().await {
                Ok(()) => println!("Muted"),
                Err(e) => println!("Mute failed: {e}"),
            },
            "2" => match session.unmute().await {
                Ok(()) => println!("Unmuted"),
